olin <- function (object, X = NA, Y = NA, alpha = seq(0.05, 1, 0.1), 
    ...
    for (i in 1:dim(maA(object))[[2]]) {
        ...
        for (ii in 1:iter) {
            x <- data.frame(summary(gcvplot(Mtmp ~ Atmp, data = data.frame(Mtmp, 
                Atmp), alpha = alpha)), alpha)
            alphaopt <- x[which(x[, 2] == min(x[, 2])), 3]
            lo <- locfit(Mtmp ~ Atmp, alpha = alphaopt)
            ...
            for (j in 1:length(alpha)) {
                for (jj in 1:length(scale)) {
                  CVA[j, jj] <- gcv(Mtmp ~ Xtmp * Ytmp, data = data.frame(Xtmp = Xtmp, 
                    Ytmp = Ytmp), alpha = alpha[j], scale = c(1, 
                    scale[jj]))[4]
                }
            }
            ...
            scaleopt <- scale[(which(min(CVA) == CVA) - 1)%/%length(alpha) + 
                1]
            lo <- locfit(Mtmp ~ Xtmp * Ytmp, alpha = alphaopt, 
                scale = c(1, scaleopt))
        ...
        if (scaling) {
            ...
            scaleoptS <- scale[(which(min(CVA) == CVA) - 1)%/%length(alpha) + 
                1]
            lo <- locfit(absMtmp ~ Xtmp + Ytmp, alpha = alphaoptS, 
                scale = scaleoptS)
